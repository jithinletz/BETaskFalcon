...
namespace BETask.Views
{
    public partial class PurchaseForm : Form
    {
        ...
        BAL.PurchaseBAL purchaseBAL = new BAL.PurchaseBAL();
        ...
        PurchaseButtonCollection button;
       int itemId = -1;
        ...
        private void ButtonActive(Enum activeEvent)
        {
            ...
            switch (activeEvent)
            {
                ...
                case EnumFormEvents.Search:
                    PurchaseSearchForm purchaseSearchForm = new PurchaseSearchForm();
        ...
        private void AutoPopulateGridColumns(int rowIndex)
        {
            try
            {
                ...
                if (discount > 0)
                {
                    discountedAmount = General.TruncateDecimalPlaces(discountedAmount - discount,4);
                }
                if (vat > 0)
                {
                    vatAmount = General.TruncateDecimalPlaces((discountedAmount * vat) / 100,4);
                }
        ...
        private void txtRoundUp_TextChanged(object sender, EventArgs e)
        {
            try
            {
                txtNetAmount.Text = Convert.ToString(General.TruncateDecimalPlaces((General.ParseDecimal(txtTaxableAmount.Text) + General.ParseDecimal(txtTotalVat.Text)) - General.ParseDecimal(txtRoundUp.Text),4));
                if (rdlCash.Checked)
                    txtCashPaid.Text = txtNetAmount.Text;
            }
            catch (Exception ee)
        ...
        private void SavePurchase()
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(errorMessage) && General.CheckFinancialDate(txtInvoiceDate.Value))
                {
                    ...
                    {
                        EDMX.purchase _purchase = GetPurchaseItem();
                        List<EDMX.purchase_item> purchaseItems = GetAllPurchaseItems();
                        int _savedId = purchaseBAL.SavePurchaseDetails(_purchase, purchaseItems);
                        UpdatePurchaseOrderAftterPurchase(_savedId);
                        General.Action($"Purchase Saved {cmbSupplier.Text} Invoice {_purchase.invoice_number} Net {_purchase.net_amount}");
                        General.ShowMessage(General.EnumMessageTypes.Success, "Purchase Successfully Saved");
                        ButtonActive(EnumFormEvents.Cancel);
                        ResetForms();
        ...
        /// </summary>
        /// <returns></returns>
        private EDMX.purchase GetPurchaseItem()
        {
            EDMX.purchase _purchaseItem = null;
            try
            {
                int bankId = 0;
                if (rdlBank.Checked)
                ...
                _purchaseItem = new EDMX.purchase()
                {
                    purchase_id = Convert.ToInt32(txtPurchaseNo.Text),
                    purchase_order = Convert.ToInt32(General.IsTextboxEmpty(txtOrder)?"0":txtOrder.Text),
                    bank_id = bankId,
                    ...
                    purchase_date = txtInvoiceDate.Value,
        ...
        /// </summary>
        /// <returns></returns>
        private List<EDMX.purchase_item> GetAllPurchaseItems()
        {
            List<EDMX.purchase_item> purchaseItems = new List<EDMX.purchase_item>();
            try
            {
                foreach (DataGridViewRow dr in dgItems.Rows)
                {
                    ...
                    {
                        purchaseItems.Add(new EDMX.purchase_item()
                        {
                            purchase_id = Convert.ToInt32(txtPurchaseNo.Text),
        ...
        private void PopulatePurchaseDetails(int purchaseId) {
            try {
                EDMX.purchase _purchase = purchaseBAL.GetPurchaseDetails(purchaseId);
                if (_purchase != null) {
                    ...
                    txtOrder.Text = Convert.ToString(_purchase.purchase_order);
                    ...
                    PopulatepurchaseItems(_purchase.purchase_item.ToList());
        ...
        {
            button = new PurchaseButtonCollection
            {
                BtnNew = btnNew,
                BtnCancel = btnCancel,
                BtnClose = btnClose,
        ...
        private void PopulatepurchaseItems(List<EDMX.purchase_item> lstPurchaseItems) {
            try {
                General.ClearGrid(dgItems);
                foreach (EDMX.purchase_item pi in lstPurchaseItems) {
        ...
        private void SearchPurchaseOrder()
        {
            try
            {
                ...
                List<EDMX.purchase_order> listPurchaseOrder = purchaseOrderBAL.SearchPurchase(General.ConvertDateServerFormat(dtpSearchLPO.Value), General.ConvertDateServerFormat(dtpSearchLPO.Value),0);
                General.ClearGrid(gridPurchaseOrder);
                if (listPurchaseOrder.Count > 0)
                {
                    foreach (EDMX.purchase_order po in listPurchaseOrder)
                    {
                        gridPurchaseOrder.Rows.Add(po.purchase_id, po.customer.customer_name, po.net_amount);
                    }
                }
            }
            catch (Exception)
        ...
        private void SearchPurchaseOrderItem(int purchaseId)
        {
            try
            {
                BAL.PurchaseOrderBAL purchaseOrderBAL = new BAL.PurchaseOrderBAL();
                List<EDMX.purchase_order_item> listPurchaseOrderItem = purchaseOrderBAL.SearchPurchaseItems(purchaseId);
                General.ClearGrid(gridOrderItem);
                if (listPurchaseOrderItem.Count > 0)
                {
                    foreach (EDMX.purchase_order_item pi in listPurchaseOrderItem)
                    {
                        gridOrderItem.Rows.Add(pi.purchase_item_id,false,pi.item_id, pi.item.item_name, pi.item.uom_setting.uom_name, pi.qty, pi.rate, pi.gross_amount, pi.discount, pi.item.tax_setting.tax_value, pi.vat_amount, pi.net_amount);
                        if (pi.original_prurchase_id !=null)
                        {
                            gridOrderItem.Rows[gridOrderItem.Rows.Count - 1].DefaultCellStyle.BackColor = System.Drawing.Color.Orange;
                        }
                    }
                }
            }
        ...
        private void linkPO_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            if (pnlImportLpo.Visible)
                pnlImportLpo.Hide();
            else
            {
                pnlImportLpo.Show();
        ...
        private void linkHideImport_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            pnlImportLpo.Hide();
        }

        private void dtpSearchLPO_ValueChanged(object sender, EventArgs e)
        {
            SearchPurchaseOrder();
        }

        private void gridPurchaseOrder_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                int purchaseId = Convert.ToInt32(gridPurchaseOrder["clmPurchaseId",e.RowIndex].Value);
                SearchPurchaseOrderItem(purchaseId);
            }
        }

        private void btnExportSelectedItems_Click(object sender, EventArgs e)
        {
            ExportOrderItemsToGrid();
            pnlImportLpo.Hide();
        }
        private void ExportOrderItemsToGrid()
        {
            try
            {
                foreach (DataGridViewRow dr in gridOrderItem.Rows)
                {
                    bool gridChecked = bool.Parse(dr.Cells["clmChkOrder"].Value.ToString());
                    if (gridChecked && dr.DefaultCellStyle.BackColor != System.Drawing.Color.Orange)
                    {
                        dgItems.Rows.Add(dr.Cells["clmOrderItemName"].Value, dr.Cells["clmOrderItmId"].Value, dr.Cells["clmOrderPacking"].Value, dr.Cells["clmOrderQty"].Value, dr.Cells["clmOrderRate"].Value, dr.Cells["clmOrderGross"].Value, dr.Cells["clmOrderDiscount"].Value, dr.Cells["clmOrderVat"].Value, dr.Cells["clmOrderVatAmount"].Value, dr.Cells["clmOrderNet"].Value);
                      
                        txtOrder.Text = gridPurchaseOrder.CurrentRow.Cells["clmPurchaseId"].Value.ToString();
                        CalucateTotals();
                    }
                }
        ...
        private void ClearOrderItemsToGrid()
        {
            try
            {
                foreach (DataGridViewRow dr in gridOrderItem.Rows)
                {
                    ...
                    if (gridChecked)
                    {
                        dr.Cells["clmChkOrder"].Value = false;
                    }
                }
        ...
        private void UpdatePurchaseOrderAftterPurchase(int purchaseId)
        {
            try
            {
                int[] purchaseOrderId =new int[gridOrderItem.Rows.Count];
                int i = 0;
                foreach (DataGridViewRow dr in gridOrderItem.Rows)
                {
                    bool gridChecked = bool.Parse(dr.Cells["clmChkOrder"].Value.ToString());
                    if (gridChecked && dr.DefaultCellStyle.BackColor != System.Drawing.Color.Orange)
                    {
                        purchaseOrderId[i] =Convert.ToInt32(dr.Cells["clmOrderId"].Value);
                        i++;
                    }
                }
                if (purchaseOrderId.Length > 0)
                {
                    BAL.PurchaseOrderBAL purchaseOrder = new BAL.PurchaseOrderBAL();
                    purchaseOrder.UpdateImportedItems(purchaseOrderId,purchaseId);
                  
                }
                ClearOrderItemsToGrid();
            }
            catch (Exception ee)
            {
                General.Error(ee.ToString());
               General.ShowMessage(General.EnumMessageTypes.Warning, "Purchase Saved Successfully , Unable to update purchase order items");
            }
        }

        private void gridOrderItem_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            if (e.ColumnIndex != 1)
                e.Cancel = true;
            else
            {
               if( gridOrderItem.CurrentRow.DefaultCellStyle.BackColor==System.Drawing.Color.Orange)
                    e.Cancel = true;
            }
        }
    }
    class PurchaseButtonCollection
    {
