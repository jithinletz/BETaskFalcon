...
namespace BETask.Views
{
    public partial class RecieptForm : Form
    {
        ...
        RecieptButtonCollection button;
        List<Model.AccountLedgerModel> listLedger;
        int transactionNumber = 0, transactionTypeId = 0;
        string transactionType = REP.AccountTransactionDAL.EnumTransactionTypes.RECIEPT.ToString();
        public RecieptForm()
        {
            InitializeComponent();
        }
            ...
            switch (activeEvent)
            {
                ...
                case EnumFormEvents.New:
                    ...
                    if (btnNew.Text.ToLower().Contains("new"))
                        txtDocumentNo.Text = DAL.DAL.DocumentSerialDAL.GetNextDocument(DAL.DAL.DocumentSerialDAL.EnumDocuments.RECIEPT.ToString()).ToString();
                    break;
        ...
        private void ResetForms()
        {
            ...
            this.transactionType = REP.AccountTransactionDAL.EnumTransactionTypes.RECIEPT.ToString();
            this.transactionTypeId = 0;
            transactionNumber = 0;
        ...
        private void Search()
        {
            PaymentSearchForm paymentSearchForm = new PaymentSearchForm(true);
            ...
            if (result == DialogResult.OK)
            {
                ...
                {
                    ButtonActive(EnumFormEvents.Cancel);
                    PopulateRecieptDetails(_transactionNumber, paymentSearchForm.copied);
        ...
        private List<EDMX.account_transaction> GetAccountTransactionList()
        {
            ...
            try
            {
                ...
                listAccount.Add(new EDMX.account_transaction
                {
                    ...
                    credit = 0,
                    debit = General.TruncateDecimalPlaces(General.ParseDecimal(txtAmount.Text)),
                    narration = txtRemarks.Text,
                    transaction_type = this.transactionType == "" ? REP.AccountTransactionDAL.EnumTransactionTypes.RECIEPT.ToString() : this.transactionType,
                    status = 1,
                    ...
                    if (General.ShowMessageConfirm() == DialogResult.Yes)
                    {
                        ...
                        if (listAccount != null && listAccount.Count > 0)
                        {
                            if (costCenterBAL.ValidateCostCenter(listAccount))
                            {
                                ...
                                string referance = "";
                                int transactionNumber = accountTransactionBAL.SaveAccountTransaction(listAccount, this.transactionType != "WALLET" ? REP.AccountTransactionDAL.EnumTransactionTypes.RECIEPT : REP.AccountTransactionDAL.EnumTransactionTypes.WALLET, ref referance);
                                General.Action($"New Reciept Saved {txtAmount.Text} {txtRemarks.Text}");
                                General.ShowMessage(General.EnumMessageTypes.Success, $"Reciept Successfully saved referance:{referance}");
                                if (chkCheque.Checked)
                                {
                                    SaveChequeDetails(transactionNumber);
                                }
                                //PopulateRecieptDetails(transactionNumber);
                                ButtonActive(EnumFormEvents.Cancel);
                            }
                            else
                            {
        ...
        private void PopulateRecieptDetails(int _transactionNumber, bool copied = false)
        {
            try
            {
                General.ClearGrid(dgItems);
                ...
                BAL.AccountTransactionBAL accountTransactionBAL = new BAL.AccountTransactionBAL();
                List<EDMX.account_transaction> listTransaction = accountTransactionBAL.GetAccountTransactionDetail(_transactionNumber, REP.AccountTransactionDAL.EnumTransactionTypes.RECIEPT.ToString());
                this.transactionTypeId = Convert.ToInt32(listTransaction[0].transaction_type_id);
                this.transactionType = listTransaction[0].transaction_type;
                ...
                foreach (EDMX.account_transaction tran in listTransaction)
                {
                    if (tran.debit > 0)
                    {
                        ...
                        txtAmount.Text = tran.debit.ToString();
                        txtLedgerAmount.Text = tran.debit.ToString();
                        txtRemarks.Text = tran.narration;
                    }
                    else
                    {
                        dgItems.Rows.Add(tran.ledger_id, tran.account_ledger.ledger_name, tran.credit, tran.voucher_number, !copied ? tran.narration : "", !copied ? tran.reference_id : "");
                    }
                }
        ...
        {
            button = new RecieptButtonCollection
            {
                BtnNew = btnNew,
                BtnCancel = btnCancel,
                BtnClose = btnClose,
        ...
        private void RecieptForm_Enter(object sender, EventArgs e)
        {
            FormLoad();
        }
        ...
        private void RecieptForm_KeyDown(object sender, KeyEventArgs e)
        {
                ...
                if (row >= 0)
                {
                    if (dgItems["clmLedgerId", row].Value != null)
                    {
                        ...
                        if (accountLedgerBAL.IsCostCenterEnabled(ledgerId))
                        {
                            ...
                            {
                                CostCenterEntryForm cost = new CostCenterEntryForm(_guid, ledgerId, ledgerName, amount, false);
