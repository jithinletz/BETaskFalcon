...
namespace BETask.Views
{
    public partial class PurchaseReturnForm : Form
    {
        ...
        BAL.PurchaseReturnBAL purchaseBAL = new BAL.PurchaseReturnBAL();
        ...
        PurchaseReturnButtonCollection button;
       int itemId = -1;
        ...
        private void ButtonActive(Enum activeEvent)
        {
            ...
            switch (activeEvent)
            {
                ...
                case EnumFormEvents.Search:
                    PurchaseSearchForm purchaseSearchForm = new PurchaseSearchForm(true);
        ...
        private void SavePurchase()
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(errorMessage))
                {
                    ...
                    {
                        EDMX.purchase_return _purchase = GetPurchaseItem();
                        List<EDMX.purchase_return_item> purchaseItems = GetAllPurchaseItems();
                        int _savedId = purchaseBAL.SavePurchaseReturnDetails(_purchase, purchaseItems);
        ...
        /// </summary>
        /// <returns></returns>
        private EDMX.purchase_return GetPurchaseItem()
        {
            EDMX.purchase_return _purchaseItem = null;
            try
            {
                int bankId = 0;
                if (rdlBank.Checked)
                ...
                _purchaseItem = new EDMX.purchase_return()
                {
                    purchase_return_id = Convert.ToInt32(txtPurchaseNo.Text),
        ...
        /// </summary>
        /// <returns></returns>
        private List<EDMX.purchase_return_item> GetAllPurchaseItems()
        {
            List<EDMX.purchase_return_item> purchaseItems = new List<EDMX.purchase_return_item>();
            try
            {
                foreach (DataGridViewRow dr in dgItems.Rows)
                {
                    ...
                    {
                        purchaseItems.Add(new EDMX.purchase_return_item()
                        {
                            purchase_return_id = Convert.ToInt32(txtPurchaseNo.Text),
        ...
        private void PopulatePurchaseDetails(int purchaseId) {
            try {
                EDMX.purchase_return _purchase = purchaseBAL.GetPurchaseReturnDetails(purchaseId);
                if (_purchase != null) {
                    cmbSupplier.Text = _lstCustomers.Where(x => x.Customer_Id == _purchase.vendor_id).FirstOrDefault().Customer_Name;
                    txtPurchaseNo.Text =Convert.ToString( _purchase.purchase_return_id);
                    purchaseId = _purchase.purchase_return_id;
                    ...
                    PopulatepurchaseItems(_purchase.purchase_return_item.ToList());
                    this.purchaseId = _purchase.purchase_return_id;
                    ButtonActive(EnumFormEvents.Update);
                  //  CalucateTotals();
                }
            }
        ...
        private void PopulatepurchaseItems(List<EDMX.purchase_return_item> lstPurchaseItems) {
            try {
                General.ClearGrid(dgItems);
                foreach (EDMX.purchase_return_item pi in lstPurchaseItems) {
        ...
        {
            button = new PurchaseReturnButtonCollection
            {
                BtnNew = btnNew,
                BtnCancel = btnCancel,
                BtnClose = btnClose,
    ...
    class PurchaseReturnButtonCollection
    {
