...
namespace BETask.Views
{
    public partial class SaleReturnForm : Form
    {
        ...
        BAL.SaleReturnBAL saleBAL = new BAL.SaleReturnBAL();
        ...
        SaleReturnButtonCollection button;
        int itemId = -1;
            ...
            try
            {
                ...
                if (cmbSupplier.SelectedItem != null)
                {
                    ...
                    if (customerId > 0)
                    {
                        ...
                        listAgreedItems = customerAggrementBAL.GetCustomerAggrements(customerId);
        ...
        private void ButtonActive(Enum activeEvent)
        {
            ...
            switch (activeEvent)
            {
                ...
                case EnumFormEvents.Search:
                    SaleSearchForm saleSearchForm = new SaleSearchForm(true);
        ...
        private void SaveSale()
        {
            try
            {
                if (General.ShowMessageConfirm() == DialogResult.Yes)
                {
                    ...
                    {
                        EDMX.sales_return sales = GetSaleItem();
                        List<EDMX.sales_return_item> saleItens = GetAllSaleItems();
                        int _savedId = saleBAL.SaveSaleReturn(sales, saleItens);
                        General.Action($"Sale Return Saved {cmbSupplier.Text} Invoice {sales.sales_return_id} Net {sales.net_amount}");
                        General.ShowMessage(General.EnumMessageTypes.Success, "Sale Return Successfully Saved");
                        ButtonActive(EnumFormEvents.Cancel);
                        ResetForms();
        ...
        /// </summary>
        /// <returns></returns>
        private EDMX.sales_return GetSaleItem()
        {
            EDMX.sales_return _sale = null;
            try
            {
                int bankId = 0;
                if (rdlBank.Checked)
                ...
                _sale = new EDMX.sales_return()
                {
                    sales_return_id = Convert.ToInt32(txtSaleNo.Text), 
                    bank_id = bankId,
                    cash_paid = General.ParseDecimal(txtCashPaid.Text),
                    cheque_no = txtCheque.Text,
                    customer_id = _lstCustomers.Where(u => u.Customer_Name == cmbSupplier.Text).Select(i => i.Customer_Id).FirstOrDefault(),
                    balance_amount = General.ParseDecimal(txtBalance.Text),
                    gross_amount = General.ParseDecimal(txtTotalBeforeVat.Text),
                    sales_date = Convert.ToDateTime(txtInvoiceDate.Text),
                    sales_number = txtSaleNumber.Text,
                    net_amount = General.ParseDecimal(txtNetAmount.Text), 
                    payment_mode = rdlBank.Checked ? rdlBank.Text : rdlCash.Checked ? rdlCash.Text : rdlCredit.Checked? rdlCredit.Text:rdlCoupon.Text,
        ...
        /// </summary>
        /// <returns></returns>
        private List<EDMX.sales_return_item> GetAllSaleItems()
        {
            List<EDMX.sales_return_item> saleItems = new List<EDMX.sales_return_item>();
            try
            {
                foreach (DataGridViewRow dr in dgItems.Rows)
                {
                    ...
                    {
                        saleItems.Add(new EDMX.sales_return_item()
                        {
                            sales_return_id = Convert.ToInt32(txtSaleNo.Text),
        ...
        private void PopulateSaleDetails(int saleId) {
            try {
                EDMX.sales_return _sale = saleBAL.GetSaleDetails(saleId);
                if (_sale != null) {
                    cmbSupplier.Text = _lstCustomers.Where(x => x.Customer_Id == _sale.customer_id).FirstOrDefault().Customer_Name;
                    txtSaleNo.Text =Convert.ToString(_sale.sales_return_id);
                    saleId = _sale.sales_return_id;
                    ...
                    if (_sale.payment_mode == "Cash")
                        rdlCash.Checked = true;
                    else if (_sale.payment_mode == "Bank")
                    {
                        GetAllBanks();
                    ...
                    if (_sale.payment_mode == "Coupon")
                        rdlCoupon.Checked = true;
                    else 
                        rdlCredit.Checked = true;

                    PopulateSaleItems(_sale.sales_return_item.ToList());
                    this.saleId = _sale.sales_return_id;
                    ButtonActive(EnumFormEvents.Update);
                  //  CalucateTotals();
                }
            }
        ...
        public void GetSupplierDetails(string supplierName) {
            ...
            try {
                ...
                if (objSupplier != null) {
                    ...
                    lblCouponBalance.Text = objSupplier.WalletBalance.ToString();
                }
            }
            catch (Exception ee)
            {
        ...
        private void PopulateSaleItems(List<EDMX.sales_return_item> lstsaleItems) {
            try {
                General.ClearGrid(dgItems);
                foreach (EDMX.sales_return_item pi in lstsaleItems) {
        ...
        {
            button = new SaleReturnButtonCollection
            {
                BtnNew = btnNew,
                BtnCancel = btnCancel,
                BtnClose = btnClose,
    ...
    class SaleReturnButtonCollection
    {
