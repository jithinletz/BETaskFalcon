...
namespace BETaskAPI.Controllers
{
    public class ItemController : ApiController
    {
        ...
        /// getting item details by category name
        [System.Web.Http.HttpGet]
        [Route("api/Item/GetItemByCategoryDescription")]

        public IHttpActionResult GetItemByCategoryDescription(string company, string location, string categoryName)
        {
            ItemDAL objItemDAL = new ItemDAL();
            Response<List<Item>> response = null;
            List<Item> lstItem = new List<Item>();
            try
            {
                lstItem = objItemDAL.GetItemDetailsByCategoryDescription(company, location, categoryName);
                if (lstItem != null && lstItem.Count > 0)
                {
                    response = new Response<List<Item>>
                    {
