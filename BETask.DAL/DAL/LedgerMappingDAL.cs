...
namespace BETask.DAL.DAL
{
    public  class LedgerMappingDAL
    {
        ...
        public enum EnumLedgerMap { PURCHASE, VATONPURCHASE,ROUNDOFF,DISCOUNTRECIEVED,CASH,SALE,DISCOUNTALLOWED,VATONSALE, COMPANYLEDGER,PDCRECIEVED,PDCISSUED,DEPOSITCOLLECTION ,COUPONBOOKLIABILITY,PETTYCASH,CUSTOMER,ONLINEPAYMENTBANK, PURCHASEGROUP ,SALEGROUP}
        ...
        public List<ledger_mapping> GetLegerMappingList(Enum mapType)
        {
            string _mapType = mapType.ToString();
            List<ledger_mapping> mapLedger = new List<ledger_mapping>();
            try
            {
                using (var context = new betaskdbEntities())
                {
                    mapLedger = context.ledger_mapping.Where(x => x.status == 1 && x.ledger_type == _mapType && x.status == 1).ToList();
                }
            }
            catch (Exception ee)
            {
        ...
        public PurchaseAccountPostModel SetPurchaseAccountLedger(PurchaseAccountPostModel purchaseAccountPosting)
        {
            try
            {
              
                List<EDMX.ledger_mapping> listLedger = GetPurchaseLegers();


                foreach (EDMX.ledger_mapping ledger in listLedger)
                {

                    if (ledger.ledger_type == LedgerMappingDAL.EnumLedgerMap.PURCHASE.ToString())
                    {
                        purchaseAccountPosting.PurchaseLedger = Convert.ToInt32(ledger.ledger_id);
                    }
                    else if (ledger.ledger_type == LedgerMappingDAL.EnumLedgerMap.PETTYCASH.ToString())
                    {
                        purchaseAccountPosting.CashPurchaseLedger = Convert.ToInt32(ledger.ledger_id);
                    }
                    else if (ledger.ledger_type == LedgerMappingDAL.EnumLedgerMap.VATONPURCHASE.ToString())
                    {
                        purchaseAccountPosting.VatOnPurchaseLedger = Convert.ToInt32(ledger.ledger_id);
                    }
                    else if (ledger.ledger_type == LedgerMappingDAL.EnumLedgerMap.ROUNDOFF.ToString())
                    {
                        purchaseAccountPosting.RoundOffLedger = Convert.ToInt32(ledger.ledger_id);
                    }
                    else if (ledger.ledger_type == LedgerMappingDAL.EnumLedgerMap.DISCOUNTRECIEVED.ToString())
                    {
                        purchaseAccountPosting.DiscountRecievedLedger = Convert.ToInt32(ledger.ledger_id);
                    }
                }
